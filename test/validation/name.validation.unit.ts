...
describe("Director name validation tests", () => {
    ...
    // title validation
        ...
        it ("should not validate previous names error if name update", async() => {
            ...
            const response = await request(app).post(UPDATE_DIRECTOR_NAME_URL)
        ...
        it ("should persist language in backlink if an error occurs", async() => {
            ...
            const response = await request(app).post(DIRECTOR_NAME_URL + WELSH_LANG)
            .set("Content-Type","application/json").send({ first_name: "§" + FIFTY_ONE_CHARACTERS });
