...
describe("Director nationality controller tests", () => {
  ...
  describe("Nationality front end Validation", () => {
    ...
    it ("should return nationality error if dual exceeds length", async() => {
      ...
      expect(response[0].messageKey).toContain(nationalityOneErrorMessageKey.NATIONALITY_LENGTH_49)
      expect(response[0].source).toContain("typeahead_input_0")
    });
    ...
    it ("should return nationality error if dual nationality exceeds length", async() => {
      ...
      expect(response[0].messageKey).toContain(nationalityOneErrorMessageKey.NATIONALITY_LENGTH_49)
      expect(response[0].source).toContain("typeahead_input_2")
      expect(response[1].messageKey).toContain(nationalityOneErrorMessageKey.NATIONALITY_LENGTH_49)
      expect(response[1].source).toContain("typeahead_input_1")
    });
    ...
    it ("should return nationality error if multiple  exceeds length 48", async() => {
      ...
      expect(response[0].messageKey).toContain(nationalityOneErrorMessageKey.NATIONALITY_LENGTH_48)
    });
    ...
    it ("should return nationality duplication error if nationality two and three are duplicates", () => {
      ...
      expect(response[0].messageKey).toContain(nationalityThreeErrorMessageKey.NATIONALITY_3_DUPLICATE)
    });
    ...
    it ("should return nationality duplication error if nationality one and two are duplicates", () => {
      ...
      expect(response[0].messageKey).toContain(nationalityTwoErrorMessageKey.NATIONALITY_2_DUPLICATE)
    });
    ...
    it ("should return nationality select from list error if nationality two not in list", () => {
      ...
      expect(response![0].messageKey).toContain(nationalityErrorMessageKey.NATIONALITY_INVALID)
    });
    ...
    it ("should return validation length error if nationality1 exceeds 50 character", () => {
      ...
      expect(response![0].messageKey).toContain(nationalityOneErrorMessageKey.NATIONALITY_LENGTH_50)
    });

    it ("should not return validation length error 50 in priory order", () => {
      const response = validateInvalidLengthValuesForNationality([SHORT_NATIONALITY], NationalityValidation);
      expect(response![0].messageKey).not.toEqual(nationalityOneErrorMessageKey.NATIONALITY_LENGTH_50);
      expect(response![0].messageKey).toContain("nationality-length");
