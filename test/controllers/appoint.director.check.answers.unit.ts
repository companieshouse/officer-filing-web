...
describe("Appoint director check answers controller tests", () => {
    ...
    describe("post tests", () => {
      ...
      it("Should redirect to unknown error stop screen if there are any validation errors", async () => {
        mockGetCurrentOrFutureDissolved.mockReturnValueOnce(false);
        ...
        expect(mockGetValidationStatus).toHaveBeenCalled();
        expect(response.text).toContain("Found. Redirecting to /appoint-update-remove-company-officer/company/12345678/cannot-use?stopType=unknown-error");
      });
