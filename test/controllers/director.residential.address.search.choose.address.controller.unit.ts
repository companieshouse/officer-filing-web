...
import { DIRECTOR_CONFIRM_RESIDENTIAL_ADDRESS_PATH, DIRECTOR_RESIDENTIAL_ADDRESS_SEARCH_CHOOSE_ADDRESS_PATH, urlParams } from "../../src/types/page.urls";
...
const PAGE_URL = DIRECTOR_RESIDENTIAL_ADDRESS_SEARCH_CHOOSE_ADDRESS_PATH
...
describe("Director residential address array page controller tests", () => {
    ...
    describe("get tests", () => {
      ...
      it("Should navigate to error page when officer filing residential address is undefined", async () => {
        mockGetOfficerFiling.mockResolvedValueOnce({});
        const response = await request(app).get(PAGE_URL);
      ...
      it("Should navigate to error page when officer filing residential address postal code is undefined", async () => {
        mockGetOfficerFiling.mockResolvedValueOnce({
          residentialAddress: {}
        });
        const response = await request(app).get(PAGE_URL);
      ...
      it("Should navigate to error page when officer filing residential address postal code is blank", async () => {
        mockGetOfficerFiling.mockResolvedValueOnce({
          residentialAddress: {
            postalCode: ""
          }
