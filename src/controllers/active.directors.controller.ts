...
async function beginUpdateJourney(req: Request, res: Response, session: Session, companyNumber: string, transactionId: string, appointmentId: any, lang: string | undefined) {
  ...
  const officerFiling: OfficerFiling = {
    ...
    nationality1: nationalities?.[0]? formatNationalitiesToSentenceCase(nationalities[0]) : "",
    nationality2: nationalities?.[1]? formatNationalitiesToSentenceCase(nationalities[1]): "",
    nationality3: nationalities?.[2]? formatNationalitiesToSentenceCase(nationalities[2]): "",
    serviceAddress: {
