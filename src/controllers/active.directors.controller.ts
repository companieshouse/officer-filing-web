...
async function beginUpdateJourney(req: Request, res: Response, session: Session, companyNumber: string, transactionId: string, appointmentId: any, lang: string | undefined) {
  ...
  const officerFiling: OfficerFiling = {
    ...
    nationality1: nationalities && nationalities.length > 0? formatNationalitiesToSentenceCase(nationalities[0]): "",
    nationality2: nationalities && nationalities.length > 1? formatNationalitiesToSentenceCase(nationalities[1]): "",
    nationality3: nationalities && nationalities.length > 2? formatNationalitiesToSentenceCase(nationalities[2]): "",
    serviceAddress: {
