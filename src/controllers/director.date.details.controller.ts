...
export const get = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const dateOfBirthFields = officerFiling.dateOfBirth ? officerFiling.dateOfBirth.split('-').reverse() : [];
        const dateOfAppointmentFields = officerFiling.appointedOn ? officerFiling.appointedOn.split('-').reverse() : [];
...
export const post = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const dobValidationError = validateDateOfBirth(dobDay, dobMonth, dobYear, DobDateValidation);
        const dateOfBirth = new Date(parseInt(dobYear), parseInt(dobMonth) - 1, parseInt(dobDay));
        const appointmentValidationError = validateDateOfAppointment(appointmentDay, appointmentMonth, appointmentYear, AppointmentDateValidation, dateOfBirth, companyProfile);
