import { UPDATE_DIRECTOR_CONFIRM_CORRESPONDENCE_ADDRESS_PATH } from './../../types/page.urls';
...
export const getCorrespondenceManualAddress = async (req: Request, res: Response, next: NextFunction, templateName: string, backLink: string, isUpdate?: boolean  ) => {
  try {
    ...
    let getBackLink = urlUtils.getUrlToPath(backLink, req);
    if (correspondenceAddressBackParam && correspondenceAddressBackParam.includes("confirm-correspondence-address")) {
      if (isUpdate) {
        ...
        backLink = urlUtils.getUrlToPath(UPDATE_DIRECTOR_CONFIRM_CORRESPONDENCE_ADDRESS_PATH, req)
      }
    }
...
export const postCorrespondenceManualAddress = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const transactionId = urlUtils.getTransactionIdFromRequestParams(req);
    const submissionId = urlUtils.getSubmissionIdFromRequestParams(req);
    const session: Session = req.session as Session;
