...
export const get = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const directorName = formatDirectorNameForDisplay(appointment);

        if (officerFiling.resignedOn) {
            const dateFields = officerFiling.resignedOn.split('-');
...
export const post = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const removeDateValidationErrors: ValidationError[] = [];
