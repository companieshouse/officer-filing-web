...
export const post = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!selectedAddress) {
    const validationError = createValidationError(correspondenceAddressErrorMessageKey.CORRESPONDENCE_ADDRESS_BLANK, [DirectorField.ADDRESS_ARRAY], ukAddresses[0]?.premise);
