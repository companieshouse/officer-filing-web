...
export const get = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    renderPage(req, res, companyNumber, officerFiling.data, undefined);
  } catch (e) {
    return next(e);
  }
};
  ...
  try {
    ...
    if (await getCurrentOrFutureDissolved(session, companyNumber)){
      return res.redirect(
        ...
        URL_QUERY_PARAM.PARAM_STOP_TYPE, STOP_TYPE.DISSOLVED));
    }
    ...
    if (!validationStatus.isValid) {
      return res.redirect(
        urlUtils.setQueryParam(urlUtils.getUrlToPath(BASIC_STOP_PAGE_PATH, req), 
        URL_QUERY_PARAM.PARAM_STOP_TYPE, STOP_TYPE.UNKNOWN_ERROR));
    }
...
const renderPage = async (req: Request, res: Response, companyNumber: string, officerFiling: OfficerFiling, errors: FormattedValidationErrors | undefined) => {
