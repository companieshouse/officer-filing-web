...
export const hasCessationDate = async (session: Session, companyNumber: string): Promise<Boolean> => {
    const client = createPublicOAuthApiClient(session);
    const ofService: OfficerFilingService = client.officerFiling;
    const response: Resource<Boolean> | ApiErrorResponse = await ofService.getHasCessationDate(companyNumber);
